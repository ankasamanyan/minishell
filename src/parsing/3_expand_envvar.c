...
in token delimitation and a $ can only by another word-char or 0.
...
-	string_c: Starts at and includes the 0 or quotationmark that delimited string_b
	and goes to the end of the string (which will ofc be the same char in case of 0).
	ft_strlen(lexeme) will always be too long, but it doesn't matter because ft_substr
	will just terminate at end of string. Only important that it doesn't terminate too soon.
-	Keeps string_a and string_c
-	Searches for the contents of string_b in at the start of each position of env.
-	If found, takes the substring of that env part. Starts strlen of string_b + 1 in order
	to skip the identifier string and the equal sign at start of env position.
-	Then just mashes the strings back together in string a
-	Frees strings b and c; lexeme get's freed in calling function
*/
char	*replace_dollar(t_par *p, char *lexeme)
{
	...
	i = dollar;
	p->string_a = ft_substr(lexeme, 0, i);
	while (lexeme[i] && !is_quotationmark(lexeme[i]))
		i++;
	p->string_b = ft_substr(lexeme, dollar + 1, i - dollar - 1);
	p->string_c = ft_substr(lexeme, i, ft_strlen(lexeme));
	i = 0;
	while (p->env[i] && ft_strncmp(p->env[i], p->string_b, ft_strlen(p->string_b)))
		i++;
	if (!p->env[i])
		p->string_b = ft_calloc(sizeof(char), 1);
	else
		p->string_b = ft_substr(p->env[i], ft_strlen(p->string_b) + 1,
				ft_strlen(p->env[i]));
	p->string_a = ft_strjoin(p->string_a, p->string_b);
	p->string_a = ft_strjoin(p->string_a, p->string_c);
	free(p->string_b);
	free(p->string_c);
	return (p->string_a);
}
